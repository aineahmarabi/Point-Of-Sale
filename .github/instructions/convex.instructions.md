...
## Clerk webhook data flow guidelines

All user data in this workspace follows the **Clerk-first** pattern:

1. **Never write user data directly to Convex.** All user updates must go through Clerk first, then the Clerk webhook (`packages/backend/convex/user/clerk.ts`) syncs the changes to Convex automatically.
2. **Update flow:** Client → `updateClerkUser()` server action → Clerk API → Clerk `user.updated` webhook → Convex `update` internal mutation.
3. **Create flow (admins):** Client → `signUp()` hook → Clerk API → Clerk `user.created` webhook → Convex `create` internal mutation.
4. **Invite flow (customers):** Client → `inviteClerkUser()` server action → Clerk Invitations API → user signs up via invitation link → Clerk `user.created` webhook → Convex `create` internal mutation.
5. **Non-native Clerk fields** (phone, user_type, role, status, app) are stored in Clerk's `public_metadata` and extracted by the webhook handler.
6. **The `is_admin` boolean** is computed server-side by the webhook: `appArray.includes("admin")`. It is indexed for efficient admin/customer table queries.
7. **Do NOT create `update` mutations** on `admins.ts` or `customers.ts` for user data. Only `remove` mutations are needed since all updates go through Clerk.
8. **Hooks available:**
   - `useUpdateUser()` from `@repo/auth/hooks` — wraps `updateClerkUser` server action
   - `useInvite()` from `@repo/auth/hooks` — wraps `inviteClerkUser` server action
   - `useSignUp()` from `@repo/auth/hooks` — wraps Clerk sign-up for admin creation

## Multi-app user model guidelines

- Users can belong to one or both apps: `"admin"` and `"web"`. The `app` field is an array (e.g. `["admin"]`, `["web"]`, `["admin", "web"]`).
- Users with `"admin"` in their app array have `is_admin: true` and appear in the admins table.
- Users without `"admin"` in their app array have `is_admin: false` and appear in the customers table.
- Use `isAdmin: true` / `isAdmin: false` in `api.user.users.list` and `api.user.users.count` queries to filter by admin/customer.
- Do NOT use the `app` array directly in index queries — use the `is_admin` boolean field with the `by_is_admin*` indexes instead.
