...
export default function Inventory() {
  ...
  const [selected, setSelected] = useState<Doc<"inventory"> | undefined>();
  const cursorsRef = useRef<Map<string, string>>(new Map());

  const handleView = useCallback((row: Doc<"inventory">) => {
    setSelected(row);
    setSheetMode("view");
    setSheetOpen(true);
  }, []);
  ...
  const handleEdit = useCallback((row: Doc<"inventory">) => {
    setSelected(row);
    setSheetMode("update");
    setSheetOpen(true);
  }, []);
  ...
  const columns = useMemo(
    () => getColumns({ onView: handleView, onEdit: handleEdit }),
  ...
  const result = useQuery(api.catalog.inventory.list, {
    paginationOpts: { numItems: pageSize, cursor },
  });
  const totalCount = useQuery(
    api.catalog.inventory.count,
    showCount ? {} : "skip",
  );
