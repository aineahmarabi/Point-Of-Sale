...
import { SupplierForm, SupplierDelete } from "./form";
...
export default function Suppliers() {
  ...
  const [selected, setSelected] = useState<Doc<"suppliers"> | undefined>();
  const cursorsRef = useRef<Map<string, string>>(new Map());

  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [toDelete, setToDelete] = useState<Doc<"suppliers"> | undefined>();

  const handleView = useCallback((row: Doc<"suppliers">) => {
    setSelected(row);
    setSheetMode("view");
    setSheetOpen(true);
  }, []);
  ...
  const handleEdit = useCallback((row: Doc<"suppliers">) => {
    setSelected(row);
    setSheetMode("update");
    setSheetOpen(true);
  }, []);

  const handleDelete = useCallback((row: Doc<"suppliers">) => {
    setToDelete(row);
  ...
  const result = useQuery(api.purchasing.suppliers.list, {
    paginationOpts: { numItems: pageSize, cursor },
    ...(activeSearch ? { search: activeSearch } : {}),
    ...(status ? { status } : {}),
  });
  const totalCount = useQuery(
    api.purchasing.suppliers.count,
    showCount
      ? {
          ...(activeSearch ? { search: activeSearch } : {}),
          ...(status ? { status } : {}),
  ...
  return (
    <main className="flex flex-1 flex-col overflow-hidden">
      ...
      <DataTable
        ...
        hiddenOnMobile={["contact_name", "email", "phone"]}
      />
      <SupplierForm
        open={sheetOpen}
