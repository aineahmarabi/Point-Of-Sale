...
import { CustomerForm, CustomerDelete } from "./form";
...
export default function Customers() {
  ...
  const [selected, setSelected] = useState<Doc<"customers"> | undefined>();
  const cursorsRef = useRef<Map<string, string>>(new Map());

  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [toDelete, setToDelete] = useState<Doc<"customers"> | undefined>();

  const handleView = useCallback((row: Doc<"customers">) => {
    setSelected(row);
    setSheetMode("view");
    setSheetOpen(true);
  }, []);
  ...
  const handleEdit = useCallback((row: Doc<"customers">) => {
    setSelected(row);
    setSheetMode("update");
    setSheetOpen(true);
  }, []);

  const handleDelete = useCallback((row: Doc<"customers">) => {
    setToDelete(row);
  ...
  const result = useQuery(api.crm.customers.list, {
  ...
  return (
    <main className="flex flex-1 flex-col overflow-hidden">
      ...
      <DataTable
        ...
        hiddenOnMobile={["email", "phone", "total_spend", "visit_count"]}
      />
      <CustomerForm
        open={sheetOpen}
